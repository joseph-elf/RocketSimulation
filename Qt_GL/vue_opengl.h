...
class VueOpenGL : public SupportADessin, protected QOpenGLFunctions {
 public:
  ...
  void dessineFusee(QMatrix4x4 const &point_de_vue = QMatrix4x4(), double hauteur = 1.5,double rayon = 0.05, bool fire = true); //dessine la toupie chinoise
  void dessineParachute(QMatrix4x4 const &point_de_vue = QMatrix4x4(), double rayon = 1.5); //dessine la toupie chinoise
  ...
  void dessineTrace(Integrable* objet); // permet de dessiner la trace d'une toupie d'index nb
  ...
  void drawFloor(QMatrix4x4 const& point_de_vue = QMatrix4x4() );
  void inverseCroched(){
      if(croched){
          psi = -40;
          omega = 0;
 ...
 private:
  ...
  double X, Y, Z, theta; //correspond aux coordonees spherique liées a la position de la camera
  bool croched;
